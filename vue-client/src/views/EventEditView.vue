...
export default {
  ...
  methods: {
    updateSportEvent () {
      EventDataService.update(this.id, this.sportEvent)
        .then(response => {
          this.updateInv(this.sportEventIndex, this.sportEvent)
          this.message = response.data.message
          this.submitted = true
        })
    },
    deleteSportEvent () {
      EventDataService.delete(this.id)
        .then(response => {
          this.message = response.data.message
          this.removeInv(this.sportEvent.index)
          this.remove(this.sportEvent.name)
  ...
  computed: {
    productIndex () {
      const index = this.inventory.findIndex((e) => {
        return e.id === this.id
      })
      return index
    }
  ...
  mounted () {
    EventDataService.get(this.id)
      .then(response => {
        this.sportEvent = response.data
      })
  }
}
</script>
